...
def evaluate(model, loader, crit, device, num_classes, monitor_metric):
    ...
    # 額外收集每個 classifier 的 logits 與 labels，之後可存 best head 的 prediction
    logits_store = {name: [] for name in names}
    labels_store = {name: [] for name in names}
    ...
    for xb, yb in loader:
        ...
        for name, logits in outputs.items():
            ...
            # 收集 logits 與 labels
            logits_store[name].append(logits.detach().cpu())
            labels_store[name].append(yb.detach().cpu())
    ...
    # pick best classifier by monitor_metric
    best_name = max(names, key=lambda n: computed[n][monitor_metric])
    best_acc = computed[best_name][monitor_metric]

    # ===== 新增：存 best classifier 的 prediction 與 labels =====
    best_logits = torch.cat(logits_store[best_name], dim=0)   # [N, C]
    best_labels = torch.cat(labels_store[best_name], dim=0)   # [N]

    # 二分類時取 positive class probability；多分類則存完整 softmax
    probs = torch.softmax(best_logits, dim=1)
    if probs.shape[1] == 2:
        preds_to_save = probs[:, 1].numpy()   # positive class probability
    else:
        preds_to_save = probs.numpy()         # multiclass: save all class probs

    np.save("val_preds.npy", preds_to_save)
    np.save("val_labels.npy", best_labels.numpy())
    # ================================================
